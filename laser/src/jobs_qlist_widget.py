...
class JobContentQListWidget(QListWidget):
    ...
    def refresh(self):
        if self.current_job_name is not None:
            self.clear()
            self.loadJob(self.current_job_name)
...
class MaterialContentQListWidget(QListWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # shortcut for the Enter button
        QShortcut(QKeySequence(Qt.Key_Return), self).activated.connect(self.materialFileEnterPressed)

    def refresh(self):
        if self.current_material_name is not None:
            self.clear()
            self.loadMaterial(self.current_material_name)

    def materialFileEnterPressed(self):
        current_item = self.currentItem()
